...
--
-- * There are display routines 'displayS' for strings,
...
module Text.PrettyPrint.Annotated.WL.Text (
  ...
  , displayS, displayT, displayIO, displayDecoratedA, displayDecorated
...
instance Pretty Text where
  pretty = mconcat . intersperse line . map text . T.splitOn "\n"
...
data Doc a
  ...
  | Text {-# UNPACK #-} !Int Text   -- invariant: text doesn't contain '\n'
  | Line
...
data SimpleDoc a
  ...
  | SText {-# UNPACK #-} !Int Text (SimpleDoc a)
...
displayDecoratedA :: (Applicative f, Monoid o)
                  ...
                  -> (Text -> f o)     -- ^ How to display a string (from document or whitespace)
                  -> SimpleDoc a
                  -> f o
displayDecoratedA push pop str = go
 where
  go SEmpty         = pure mempty
  go (SChar c x)    = str (T.singleton c) <++> go x
...
displayDecorated :: Monoid o
                 ...
                 -> (Text -> o)     -- ^ How to display a string (from document or whitespace)
                 -> SimpleDoc a
                 -> o
...
displayIO handle = displayDecoratedA cpu cpu (T.hPutStr handle)
...
displayS = displayDecoratedA ci ci ((++) . T.unpack)
 where ci = const id
...
displayT = TL.toLazyText . displayDecorated cm cm TL.fromText
...
displaySpans :: Monoid o => (Text -> o) -> SimpleDoc a -> (o, SpanList a)
displaySpans str = go 0 []
 where
  go _ []          SEmpty          = (mempty, [])
  go i stk         (SChar c x)     = first (mappend $ str $ T.singleton c) $ go (i+1) stk x
